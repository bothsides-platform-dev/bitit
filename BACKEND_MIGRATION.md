...
- 화면 명세(`SCREEN_DESIGN.md` §11.5 P1~P11)와 `PG_RFQ_SPEC.md` 15개 정책을 그대로 보존
...
| **Buyer/PG 분기** | **P6에서 사용자가 라디오로 직접 선택**. 도메인 자동 추론 없음. 단, `/invite/rfq/:token` 경유는 P6 전체 스킵 + 강제 PG 경로 (도메인의 PG ws 있으면 auto-join, 없으면 신규 PG ws 생성) |
| **bizProfile 보관** | Workspace 생성 시 1회 캡처(buyer만), RFQ 생성 시마다 스냅샷 row 신규 insert. `biz_profiles`는 immutable rows |
| `BizProfile` 슬림화 | `bizNo`/`taxType`/`status`/`grade`/`gradeSource`/`gradeConfirmedBy`/`gradeConfirmedAt`만. `name`/`ceoName`/`ksic`/`mailOrderNo`/`estimatedRevenue`/`revenueYear`/`niceLookedUpAt` 제거 |
| 회사명 | `Workspace.name` 사용 |
| Grade 입력 | 사용자 5단계 직접 선택. `gradeSource ∈ {'user_confirmed', 'user_overridden'}` (`auto_nice` 제거) |
...
- `workspaces` (id, type `workspace_type`, name, domain UNIQUE WHERE NOT NULL, biz_profile_id FK→`biz_profiles.id` nullable, created_at)
- `workspace_members` (workspace_id FK, user_id FK, role `member_role`, joined_at, last_seen_at; PK(ws,user))
- `biz_profiles` (id, biz_no, tax_type `tax_type`, status `biz_status`, grade `merchant_grade`, grade_source `grade_source`, grade_confirmed_by FK→users nullable, grade_confirmed_at, created_at) — **immutable: 변경 시 새 row insert + workspace.biz_profile_id 갱신**
- `rfqs` (id text PK = `Q-YYMM-NNNN`, buyer_ws_id FK, biz_profile_id FK NOT NULL → 스냅샷, title, memo, allowed_pg_emails text[], deadline timestamptz, status `rfq_status`, awarded_bid_id FK→bids nullable, created_by FK→users, created_at, sent_at)
- `rfq_invitations` (id, rfq_id FK, pg_email, pg_ws_id FK→workspaces nullable, accepted_by_user_id FK→users nullable, token_hash UNIQUE, sent_at, opened_at, expires_at, status `invitation_status`)
...
[ 새 워크스페이스 ]   [ 코드로 합류 ]
  ...
  [BUYER 분기]
    ...
    BizLookupField (bizNo 입력 → NTS 조회 → status/taxType 표시)
    GradeConfirmPanel (5단계 라디오, source='user_confirmed')
    [만들기]
...
- `components/rfq/GradeConfirmPanel.tsx`: NICE 호출 제거(`lookupNiceGrade` 의존 삭제), 5단계 라디오만. 각 등급 옆에 법정 매출 구간 도움말(`STATUTORY_CARD_FEE`로 카드료 자동 표시 — `general`은 NaN이므로 "협상" 표시). source는 항상 `'user_confirmed'`
- `components/auth/WorkspaceTypeRadio.tsx` 신규: buyer/PG 라디오 (Korean Editorial 스타일)

P6 submit → `signupCompleteAction({ wsKind, name, bizProfile? })` (PG 분기 시 bizProfile undefined). PG 도메인이 이미 PG ws로 존재하면 join + 안내 ("toss.im 워크스페이스에 합류했습니다").
...
- `createRfqAction.ts`: `requireBuyerSession()`. 입력 = `{ title, memo, deadline, allowedPgEmails, rfpAttachmentIds, gradeOverride? }`. **bizNo는 사용자 입력 받지 않음** — workspace의 `biz_profile_id`에서 끌어옴. tx:
  1. `nextRfqId(tx)`로 ID 발급
  2. workspace.biz_profile를 읽어 **새 biz_profiles row insert** (스냅샷; gradeOverride 있으면 `gradeSource='user_overridden'` + `gradeConfirmedBy/At` 기록)
...
- `bid/submitBidAction.ts`: `requirePgSession()` + `invitationRepo.canAccess(rfqId, userId)` 가드. **STATUTORY_CARD_FEE 서버 강제**:
  ```ts
  const rfq = await rfqRepo.findById(rfqId);
  const grade = (await bizProfileRepo.findById(rfq.bizProfileId)).grade;
  const cardFees = grade === 'general' ? input.cardFeesByIssuer : null;
  // 영세/중소1~3에서는 클라이언트 입력 무시
  ```
...
- `lib/server/outbox/templates/{authVerify,authReset,authEmailChange,rfqInvited,rfqSent,bidSubmitted,rfqAwarded}.tsx`: react-email. 모든 numeric은 mono+tabular-nums (DESIGN.md), 본문 헤어라인, `— FIN —` 마킹
