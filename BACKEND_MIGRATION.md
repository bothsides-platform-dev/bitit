...
- `rfqs` (id text PK = `Q-YYMM-NNNN`, buyer_ws_id FK, biz_profile_id FK **nullable** → 스냅샷 (사업자번호·등급 모두 미입력 RFQ는 NULL), title, memo, allowed_pg_emails text[], deadline timestamptz, status `rfq_status`, awarded_bid_id FK→bids nullable, created_by FK→users, created_at, sent_at)
