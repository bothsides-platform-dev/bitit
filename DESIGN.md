...
3. **편집 마킹** — 섹션 시리얼(`01 / 14`), 그림 번호(`FIG. 03`), 이슈 번호(`№ 042`), 발행일 캡스 라인 등 **편집물의 메타 마킹**을 UI에 차용해 정보 위계를 강화한다.
...
- 본문 카피 + mono uppercase 마킹 `— FIN —` 으로 종결
- CTA 버튼 1개
...
- 빈 상태: 봉투 라인 SVG + "새로운 알림이 없습니다." + `— FIN —`
...
- **종결 마킹**: 안내 화면 하단 `— FIN —` (모노, uppercase, `--color-ink-soft`)
