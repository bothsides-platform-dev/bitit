...
기존 `app/(app)/home/page.tsx`의 buyer 콘텐츠(KPI strip + active RFQs 섹션)를 RSC 컴포넌트로 분리한다. RSC이므로 별도 단위 테스트 없음 — Task 4의 통합 검증으로 대체.
...
2. Buyer 계정 로그인 → 기존 buyer home(KPI 3개 + 진행 중 견적 목록) 정상 렌더링 확인
